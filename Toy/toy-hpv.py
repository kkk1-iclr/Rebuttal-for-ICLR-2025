...
#tools
def cg(a, b, c, x, y, num_steps):
    r = b - a
    p = r.clone()
    rs_old = torch.dot(r.view(-1), r.view(-1))  
    for _ in range(num_steps):
        Ap = f_yy(x,y,p)
        alpha = rs_old / torch.dot(p.view(-1), Ap.view(-1))  
        x = x + alpha * p
        r = r - alpha * Ap
...
def TN(x, y, b, K: int = 1 , inner_lr: float = 0.1) -> torch.Tensor:   
    p = v = b.clone()
...
for x_itr in range(x_loop):
    t0 = time.time()
    y,et= sr(x,y,tol=1/(100*(x_itr+1)),step=5,maxiter_hg=20,m=30,h0=1,ex_up=True)
...
plt.savefig('xdisaa.pdf', dpi=300, bbox_inches='tight')
...
plt.savefig('dxa.pdf', dpi=300, bbox_inches='tight')
...
plt.savefig('xdisa.pdf', dpi=300, bbox_inches='tight')
...
plt.savefig('ydisa.pdf', dpi=300, bbox_inches='tight')
...
plt.axis('off')
plt.savefig('toylegenda.pdf', dpi=300, bbox_inches='tight')